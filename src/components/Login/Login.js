...
export default function Login(props) {
    ...
    const onSubmit = (data) => {
        let allData = userList.map(user => (user.username))
        let allDataPassword = userList.map(user => (user.password))

        if ((allData.indexOf(data.firstName) !== -1) && (allDataPassword.indexOf(data.password) !== -1)) {
            debugger
            console.log(props)
            props.test()
